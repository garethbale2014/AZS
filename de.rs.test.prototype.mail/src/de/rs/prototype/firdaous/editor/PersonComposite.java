...
public class PersonComposite extends EditorComposite {
  ...
  protected DataBindingContext initDataBindingContext() {
    ...
    IObservableValue decedeLastNameValue = PojoProperties.value("lastname").observe(
            order.getPair().getPerson());

    IObservableValue decedeFirstNameWidget = SWTObservables.observeText(firstnameText, SWT.Modify);
    IObservableValue decedeFirstNameValue = PojoProperties.value("firstname").observe(
            order.getPair().getPerson());

    IObservableValue streetWidget = SWTObservables.observeText(streetText, SWT.Modify);
    IObservableValue streetValue = PojoProperties.value("street").observe(
            order.getPair().getPerson().getAddresse());

    IObservableValue houseNumberWidget = SWTObservables.observeText(houseNumberText, SWT.Modify);
    IObservableValue houseNumberValue = PojoProperties.value("houseNumber").observe(
            order.getPair().getPerson().getAddresse());

    IObservableValue cityTextNameWidget = SWTObservables.observeText(cityText, SWT.Modify);
    IObservableValue cityTextValue = PojoProperties.value("city").observe(
            order.getPair().getPerson().getAddresse());

    IObservableValue zipWidget = SWTObservables.observeText(zipText, SWT.Modify);
    IObservableValue zipValue = PojoProperties.value("zip")
            .observe(order.getPair().getPerson().getAddresse());

    IObservableValue birthCityTextWidget = SWTObservables.observeText(birthCityText, SWT.Modify);
    IObservableValue birthCityTextValue = PojoProperties.value("birthCity").observe(
            order.getPair().getPerson());

    IObservableValue professionWidget = SWTObservables.observeText(professionText, SWT.Modify);
    IObservableValue professionValue = PojoProperties.value("profession")
            .observe(order.getPair().getPerson());
    ...
    IObservableValue partnerFirstNameValue = PojoProperties.value("firstname").observe(
            order.pair.getPartner());

    IObservableValue partnerProfessionWidget = SWTObservables.observeText(partnerProfessionText, SWT.Modify);
    IObservableValue partnerProfessionValue = PojoProperties.value("profession").observe(
            order.pair.getPartner());
    IObservableValue partnerStreetWidget = SWTObservables.observeText(partnerStreetText, SWT.Modify);
    IObservableValue partnerStreetValue = PojoProperties.value("street").observe(
            order.getPair().getPartner().getAddresse());

    IObservableValue partnerHouseNumberWidget = SWTObservables.observeText(partnerHouseNrText, SWT.Modify);
    IObservableValue partnerHouseNumberValue = PojoProperties.value("houseNumber").observe(
            order.getPair().getPartner().getAddresse());

    IObservableValue partnerCityTextNameWidget = SWTObservables.observeText(partnerCityText, SWT.Modify);
    IObservableValue partnerCityTextValue = PojoProperties.value("city").observe(
            order.getPair().getPartner().getAddresse());

    IObservableValue partnerZipWidget = SWTObservables.observeText(partnerZIPText, SWT.Modify);
    IObservableValue partnerZipValue = PojoProperties.value("zip").observe(
            order.getPair().getPartner().getAddresse());
